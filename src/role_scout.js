...
function haveNotSeen(creep, room) {
  ...
  return (!Memory.rooms[room] || Memory.rooms[room].lastSeen <= (Game.time - config.basic.creeps.scout.intervalBetweenRoomVisit) ) &&
    creep.memory.search.seen.indexOf(room) === -1 &&
...
roles.scout.breadthFirstSearch = function(creep) {
  ...
  let costMatrix = function(roomName) {
      if (!Memory.rooms[roomName] || (Memory.rooms[roomName].lastSeen < (Game.time - 5))) {
          return creep.room.getCostMatrixCallback(targetPosObject, true, false, true)
      }
      return false;
    };
...
let enemys = creep.room.getEnemys();
  if (!enemys.length && creep.memory.last && creep.memory.last.pos3 && creep.pos.roomName !== creep.memory.last.pos3.roomName) {
    creep.moveTo(25, 25);
    return true;
  }

  if (enemys.length) {
    creep.say('Affraid', true);
  ...
  if (search.path.length === 0 || (creep.inBase() && creep.room.memory.misplacedSpawn)) {
    ...
    config.advanced.debug.scout && creep.log(creep.pos + ' ' + targetPosObject + ' ' + JSON.stringify(search));
    if (creep.isStuck() && onBorder(creep)) {
      creep.say('imstuck at the border', true);
      if (config.basic.creeps.scout.skipWhenStuck) {
